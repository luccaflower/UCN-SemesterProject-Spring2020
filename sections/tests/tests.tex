...
\section{Test cases}
For at gå systematisk til værks, skal der opsættes et skema med test cases\cite{Heumann}. Der bliver altså lavet flere scenarier som først blive sat op i skemaet, så testet i programmet, og til sidst vil der foretages ændringer i koden som kan fikse de opståede problemer, hvorefter de samme tests køres igen. Tests er foretaget på version: 57a8dfa af programmet, og vores test case-skemaer ses herunder.
...
Integration tests er tests som beskriver og tester hvordan 2 lag interager med hinanden. Et godt eksempel er test af DAO objekter\cite{DAO}, eftersom både metoden på DAO objektet, og dens interaktion med databasen bliver testest i disse test cases.
