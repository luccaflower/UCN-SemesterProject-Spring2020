...
Disse tests beskriver vores regressions algorithme, for at teste dette skal der bruges et massivt mængde data, vi bruger derfor en csv-fil som beskriver relationerne mellem en helt masse forskellige typer cement, denne csv-fil kan findes på \texttt{res/concrete.csv}, og selve test koden kan findes på \texttt{src/hjem/is/test/RegressionTest.java}. I de her test bruges en custom assert som vi selv har lavet, kaldet \texttt{assertApproxEquals}, dette bruges eftersom at regression halder ikke om at være 100\% perfect, men bare godt nok til det punkt at de er brugbart

først hentes concrete.csv filen ind i en \texttt{@BeforeAll} eftersom at alle test bruger den samme data, data'en er lavet om til \texttt{Observation} objekter, eftersom at det er den type som vores implementation bruger som argument

begge tests tester at algoritmet kan forudsige at \texttt{compressivestrength} på index 30 passer index for en delta af 20 af den faktiske værdi, bægger test printer også deres $r^{2}$ værdi og deres precision

den første test tester \texttt{GradientDescentFit} implementation, der bruges en alpha af 0.0001, den anden tester \texttt{NormalEquationFit}
