...
public List<PeriodicPlan> getByStoragePlan(StoragePlan storagePlan) throws DataAccessException {
    ...
    \end{minted}
    \caption{Eksempel på ingen brug af parallelitet\label{lst:getByStoragePlan}}
\end{listing}
...
UI er bygget sådan, at når programmet åbnes, kommer der et vindue frem som viser en liste over nuværende lagerplaner, og en knap "Generer ny lagerplan" som fører brugeren videre til endnu et vindue.
"Nuværende lagerplaner:" er tom fra starten, men som man tilføjer nye lagerplaner, kommer de oprettede lagerplaner frem på listen.
Når der er oprettet en lagerplan på listen, kan man trykke på planen for at gå ind og redigere den.

Det nye vindue som "Generer ny lagerplan" åbner, viser brugeren en periodeplansliste for den navngivede plan, samt et tekstfelt, hvor man kan navngive sin plan.
...
"TIlføj produkt" knappen åbner op for et lille vindue, hvori der er en liste over de is man kan vælge. derefter trykker man enden "OK" eller "Cancel". Den udvalgte is kommer frem på listen under "Produkter:"
Når brugeren er færdig trykker man på "Gem" knappen, og lukker vinduet.
